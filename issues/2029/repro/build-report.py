...
<body><main>
  ...
  <table><tr><th>Claim from the issue</th><th>Status</th><th>Evidence</th></tr>
    ...
    <tr><td><code>server.js</code> is never rebuilt by this path; only <code>app.*</code>.</td><td class="ok">Verified</td><td>Path installs load the backend from source ({link(RUNTIME, 1027, 1040, "resolveServerEntry")}); only <code>buildPluginApp</code> (and <code>buildPluginHost</code> when <code>bb.host</code> is declared) runs on load. Observed: in steps 1, 4 and 5 the <code>dist/server.js</code> mtime stays at its last <code>bb plugin build</code>/<code>git checkout</code> value while <code>dist/app.*</code> are rewritten next to it (in step 2 it changes only because <code>git checkout</code> wrote it, before the reload).</td></tr>
  ...
  <h2>3. Environment</h2>
  <ul>
    <li>bb monorepo at <code>c7c66423d</code> (branch main, 2026-08-20); origin/main (<code>f4ab03f46</code>) has three newer commits, all mobile UI (#2032, #2033, #2034), none touching <code>apps/server/src/services/plugins</code>, <code>apps/server/src/routes/plugins.ts</code>, <code>apps/cli/src/commands/plugin.ts</code> or <code>packages/plugin-build</code>. App version <code>0.39.0</code> (dev build reports <code>0.0.0-dev</code>), plugin SDK <code>0.4.9</code>. Issue was filed against 0.39.0 / SDK 0.4.1 on macOS.</li>
    <li>Linux 7.0.0-29-generic, Node v24.18.0, pnpm workspace, esbuild 0.28.1.</li>
    <li>Own dev instance via <code>scripts/bb-dev-app current</code>: server <code>http://localhost:22378</code>, app <code>http://localhost:14378</code>, host daemon <code>127.0.0.1:30378</code>, data dir <code>~/.bb-dev/projects-bb-.claude-worktrees-wf_926b3193-f6c-17-5f109972db34</code> (deleted at cleanup). The logs below are from this instance (revision run); the first run on a sibling worktree (:19347) produced the same results.</li>
    <li>Fixture plugin: <code>/tmp/bb2029-plugin</code> (git repo with <code>dist/</code> committed), assembled by <a href="2029/repro/step0-fixture-setup.sh">step0-fixture-setup.sh</a> from <a href="2029/repro/">2029/repro/</a> (<code>fixture-package.json</code> → <code>package.json</code>, <code>fixture-server.ts</code> → <code>src/server.ts</code>, <code>fixture-app.tsx</code> → <code>src/app.tsx</code>, <code>fixture-provider-marks.ts</code> → <code>src/provider-marks.ts</code>).</li>
  </ul>
  ...
  <p><strong>How to run the steps.</strong> Start your own dev instance from a bb checkout at <code>c7c66423d</code> (<code>pnpm install --frozen-lockfile && pnpm exec turbo run build && scripts/bb-dev-app current</code>), then in the shell that runs the steps:</p>
  {code('''export BB_WORKTREE=/path/to/your/bb/checkout          # the checkout whose scripts/bb-dev-app current is running
eval "$($BB_WORKTREE/scripts/bb-dev-app env)"          # BB_SERVER_URL, BB_HOST_DAEMON_PORT, BB_PROJECT_ID
# optional overrides: BB_CLI (default: node $BB_WORKTREE/packages/scripts/dist/commands/run-cli.js)
#                     BB_DEV_LOG (default: the dev.log path printed by `$BB_WORKTREE/scripts/bb-dev-app status`)
bash 2029/repro/step0-fixture-setup.sh''')}
  <p>Steps 3, 4 and 5 each need a server that has no hung service from the previous step: run <code>pnpm dev:stop && scripts/bb-dev-app current</code> in the checkout and wait ~15 s before the step. Every step prints what it does and the CLI exit codes where those are the point.</p>
  <ol start="0">
    <li><strong>Assemble, build and commit the fixture</strong> — <a href="2029/repro/step0-fixture-setup.sh">step0-fixture-setup.sh</a>. Copies the four fixture files into <code>/tmp/bb2029-plugin</code> (<code>src/provider-marks.ts</code> says <code>provider-mark-v1</code>), runs <code>bb plugin build</code>, commits everything including <code>dist/</code>. Expected and actual: <code>git status</code> clean.
    {pre("step0-fixture-setup.log")}</li>
    ...
    <li><strong>Reload rebuilds into the root when a source is newer</strong> — <a href="2029/repro/step4-reload-rebuild.sh">step4-reload-rebuild.sh</a> (fresh server). Part (a) reproduces: <code>touch src/provider-marks.ts</code> + reload rewrote <code>dist/app.*</code> (and left <code>dist/server.js</code> untouched), log line <code>rebuilding frontend bundle (plugin source is newer than dist/app.js)</code>. Part (b) did not fire in this run only because the same reload had already left the plugin hung (hung check precedes the rebuild); it is re-run cleanly as step 5.
    {pre("step4-reload-rebuild.log")}</li>
    <li><strong>SDK-version trigger defeats the mtime workaround</strong> — <a href="2029/repro/step5-sdk-mismatch.sh">step5-sdk-mismatch.sh</a>. Committed <code>app.meta.json</code> stamped <code>sdkVersion 0.4.1</code> (the reporter's SDK), every <code>dist/*</code> touched newer than every source, server restarted. Actual: rebuilt on load anyway (<code>dist/app.*</code> rewritten, <code>dist/server.js</code> untouched); checkout dirty. The log says <code>built with SDK unknown</code> rather than <code>0.4.1</code> because the simulated meta only changed the top-level <code>sdkVersion</code>, and {link("apps/server/src/services/plugins/app-bundle.ts", 244, 249, "parsePluginArtifactMeta")} rejects a meta whose <code>builtWith.pluginSdkVersion</code> (still 0.4.9) differs from <code>sdkVersion</code>, so <code>readPluginAppBundleMeta</code> returns <code>null</code>; a genuine 0.4.1-built artifact would parse and log <code>built with SDK 0.4.1</code>. Either way <code>meta?.sdkVersion !== PLUGIN_SDK_VERSION</code> is true and the rebuild fires.
  ...
  <ul>
    ...
    <li><strong>Load</strong> (server start, reload, enable): {link(RUNTIME, 1085, 1124, "plugin-runtime.ts:1085-1124")} — <code>loadAppBundleCandidate</code> rebuilds when <code>meta?.sdkVersion !== PLUGIN_SDK_VERSION</code> (<em>any</em> difference, including a meta it cannot parse — <code>readPluginAppBundleMeta</code> returns <code>null</code> for a missing or malformed <code>app.meta.json</code> or one whose <code>builtWith.pluginSdkVersion</code> disagrees with <code>sdkVersion</code>, which is why step 5 logs <code>built with SDK unknown</code>) or when {link(RUNTIME, 976, 1012, "isMutableAppBundleStale")} finds any file or directory (excluding <code>dist</code>, <code>node_modules</code>, <code>.git</code>) with <code>mtimeMs &gt; dist/app.js mtimeMs</code>. The docstring states the policy: "Mutable plugin app artifacts are only a cache of their source tree."</li>
...
scripts/bb-dev-app current            # server :22378, app :14378, host daemon :30378
export BB_WORKTREE=$PWD && eval "$(scripts/bb-dev-app env)"
bash 2029/repro/step0-fixture-setup.sh    # assemble fixture at /tmp/bb2029-plugin, bb plugin build, commit dist
...
pnpm dev:stop && scripts/bb-dev-app current   # step4(b) already committed sdkVersion 0.4.1 and touched dist/*
...
f4ab03f46 Port the web sound-wave recording bar to the mobile composer (#2034)
c4a3dc5fb Match mobile unread divider to web styling (#2033)
5f4172be3 Remove the working status from the mobile thread header (#2032)
$ git log c7c66423d..origin/main --oneline -- apps/server/src/services/plugins apps/server/src/routes/plugins.ts apps/cli/src/commands/plugin.ts packages/plugin-build/src
(empty)''')}
  <h3>Verification</h3>
  <p>An independent verifier followed the five step scripts and the vitest repro on their own worktree at <code>c7c66423d</code> and dev instance (server :20803) and got the same outcomes: install rewrote <code>dist/app.*</code> and left <code>M dist/app.meta.json</code>; checkout + reload produced identical mtimes, no rebuild, <code>degraded</code> with exit 0; the orphan step gave <code>{{"ok": true}}</code>, exit 0, <code>unknown command 'collab'</code> and a growing count of <code>The database connection is not open</code> (20 → 24 → 27 across disable/enable); <code>touch src</code> + reload logged <code>plugin source is newer than dist/app.js</code>; the SDK-mismatch restart logged <code>built with SDK unknown, running SDK is 0.4.9</code>; the vitest file failed at the asserted line. They also checked every code permalink and the release check (<code>dbbb7640f</code> not in <code>desktop-v0.39.0</code>). The verifier's findings and what changed in this revision:</p>
  <ul>
    <li><strong>Major — repro not copy-pasteable</strong> (fixture assembly only hinted at; step scripts defaulted to the author's worktree path and launcher log; saved <code>fixture-provider-marks.ts</code> said <code>v2</code> so step 2's <code>sed</code> was a no-op). Fixed: added <a href="2029/repro/step0-fixture-setup.sh">step0-fixture-setup.sh</a> which copies the fixture files into place (provider mark reset to <code>v1</code>), builds and commits; every step script now derives the CLI and dev log from <code>BB_WORKTREE</code> (with <code>BB_CLI</code>/<code>BB_DEV_LOG</code> overrides) and the "How to run the steps" paragraph above documents that. All six steps and the vitest repro were re-run from scratch on a fresh worktree/instance (:22378) with the revised scripts; the logs embedded above are from that run and match the first run.</li>
    <li><strong>Minor — "server.js mtime unchanged across all steps" was loosely evidenced.</strong> Fixed: steps 4 and 5 now print <code>dist/server.js</code> alongside <code>dist/app.*</code> (untouched while <code>app.*</code> are rewritten), and the claims-table wording points at the code path plus those observations, noting that step 2's <code>server.js</code> mtime change came from <code>git checkout</code>.</li>
    <li><strong>Minor — "built with SDK unknown" vs committed <code>sdkVersion 0.4.1</code> unexplained.</strong> Fixed: step 5 and the root-cause section now explain that <code>parsePluginArtifactMeta</code> rejects the hand-edited meta (<code>builtWith.pluginSdkVersion</code> 0.4.9 ≠ <code>sdkVersion</code> 0.4.1), so <code>meta</code> is <code>null</code>; a real 0.4.1 artifact would log <code>0.4.1</code>, and the rebuild fires either way.</li>
    <li>Also refreshed: environment ports/data dir, and the origin/main check (now three newer mobile-UI commits, none touching the plugin paths).</li>
  </ul>
</main></body></html>
"""
OUT.write_text(doc)
