...
const sample = (label) => page.evaluate((label) => {
  ...
  return { label, nodes: document.getElementsByTagName("*").length, rows: document.querySelectorAll("[data-timeline-row-id]").length, realized: document.querySelectorAll("[data-timeline-row-realized=\"true\"]").length, windowed: !!document.querySelector("[data-timeline-windowed]"), scrollHeight: el.scrollHeight, scrollTop: el.scrollTop, longTasks: window.__bb1301.longTasks.length };
}, label);
...
for (let i = 1; i <= 60; i++) {
  ...
  if (!tookMid && s.rows > 300) { tookMid = true; console.log(await saveScreenshot(await page.screenshot(), "1301-pr1384-mid-scroll.png")); }
...
console.log(await saveScreenshot(await page.screenshot(), "1301-pr1384-top-of-history.png"));
