...
await page.goto("http://localhost:12702/projects/proj_8q7r7ymyyp/threads/thr_ndvdqterge");
...
for (let i = 0; i < 40; i++) {
  ...
  if (original > 0 && !docsShot) {
    docsShot = true;
    await saveScreenshot(await page.screenshot(), "1773-verify-diff-docs-open.png");
  }
  const t = await page.locator("[data-sonner-toast]").allInnerTexts();
  if (t.length > 0 && t.join("").trim() !== "" && !shot) {
    shot = true;
    toastText = t.join("\n---\n");
    await saveScreenshot(await page.screenshot(), "1773-verify-diff-toast.png");
  }
}
console.log("STRIP TIMELINE:\n" + timeline.join("\n"));
console.log("TOAST:", JSON.stringify(toastText));
console.log("REQS:", JSON.stringify(reqs));
await saveScreenshot(await page.screenshot(), "1773-verify-diff-after.png");
await page.reload();
await page.waitForTimeout(6000);
...
await saveScreenshot(await page.screenshot(), "1773-verify-diff-reload.png");
