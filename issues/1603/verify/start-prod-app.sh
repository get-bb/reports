...
LOG=/tmp/bb-reports/issues/1603/verify/prod-app.log
mkdir -p "$DATA"
...
BB_DISABLE_AUTO_UPDATE=1 nohup node packages/bb-app/dist/bb-app.js \
  ...
  > "$LOG" 2>&1 &
echo $! > /tmp/bb-reports/issues/1603/verify/prod-app.pid
for i in $(seq 1 40); do
  if curl -fs -o /dev/null "http://localhost:$SP/"; then echo "up: http://localhost:$SP (pid $(cat /tmp/bb-reports/issues/1603/verify/prod-app.pid))"; exit 0; fi
  sleep 1
done
