...
if (CFG.hook !== false) await cdp.send("Page.addScriptToEvaluateOnNewDocument", { source: `(() => {
...
const metricsDelta = {}; for (const k of ["ScriptDuration","LayoutDuration","RecalcStyleDuration","TaskDuration","LayoutCount","RecalcStyleCount"]) metricsDelta[k] = /Duration/.test(k) ? Math.round((m1[k] - m0[k]) * 1000) : m1[k] - m0[k];
const res = await page.evaluate(() => { window.__bbTallyOn = false; return { commits: window.__bbCommits || 0, rendered: window.__bbRendered || {}, renderedTotal: window.__bbRenderedTotal || 0, providers: window.__bbProviders || {}, probes: window.__bbRenderCounts, longTasks: window.__bbLong }; });
// profile aggregation
...
const out = {
  ...
  metricsDelta, cpuBusyMs: Math.round(total - idle), cpuBusyPerKeyMs: Math.round((total - idle) / N * 10) / 10,
...
if (CFG.shot) console.log("shot:", await saveScreenshot(await page.screenshot(), CFG.shot));
