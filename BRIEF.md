<!-- Placeholders: get-bb/bb owner/name · 1bc80e1d57991fe60a3896e60e0159427194c7bb / 1bc80e1d5 base commit · 2026-09-10 · /tmp/slopcop-3408-reports.pPH6eo report work dir (clone of get-bb/reports) · No live provider used; local MCP SDK client only from `bb provider list`. Fill them, save as /tmp/slopcop-3408-reports.pPH6eo/BRIEF.md. -->
# Direct single-issue investigation brief

You are investigating ONE GitHub issue in get-bb/bb and producing an HTML bug report that lets a reader who knows nothing about the codebase reproduce the bug themselves. Base commit: 1bc80e1d57991fe60a3896e60e0159427194c7bb (branch main as of 2026-09-10).
...
- Run only the trusted target repository at `1bc80e1d57991fe60a3896e60e0159427194c7bb` or a later trusted `origin/main` commit.
...
## Your environment
- You are in your OWN git worktree of the bb monorepo (check `pwd`, `git rev-parse HEAD`). Nobody else touches it. You may edit source, add instrumentation, and add test files. Never check out a pull request branch. Return to the base commit (`git checkout 1bc80e1d5`) if you need the pristine tree again.
...
- Providers available on this machine: No live provider used; local MCP SDK client only (see `bb provider list`). Real turns cost real usage; keep prompts tiny ("Reply only with ok.").
- Screenshots / browser driving: load the `doobie` skill via the Skill tool (doobie CLI). Save PNGs to /tmp/slopcop-3408-reports.pPH6eo/issues/assets/<ISSUE>-<name>.png and reference them as `assets/<ISSUE>-<name>.png`. For visual bugs, capture BEFORE and the moment the bug shows, and a shot of you triggering it (the command/click).
...
- Do NOT touch ~/.bb (the user's real data dir) or the user's real bb instance on :38886. Do NOT push branches or open PRs. Do NOT comment on GitHub. Only write files under /tmp/slopcop-3408-reports.pPH6eo/ and inside your worktree.
...
- File hygiene: write ONLY /tmp/slopcop-3408-reports.pPH6eo/issues/<ISSUE>.html, /tmp/slopcop-3408-reports.pPH6eo/issues/<ISSUE>/** (repro files, logs, diffs) and /tmp/slopcop-3408-reports.pPH6eo/issues/assets/<ISSUE>-*.png. Never write loose files in /tmp/slopcop-3408-reports.pPH6eo/issues/ root. If files already exist at those paths (a previous attempt), keep their names — never move, rename, or delete existing artifacts; overwrite in place only if you have a better version.
- Base commit vs origin/main: your worktree is at 1bc80e1d5. Run `git fetch origin main` and check whether a later commit on origin/main already fixes the bug (`git log 1bc80e1d5..origin/main --oneline -- <paths>`); if so, verdict ALREADY FIXED and name the commit, but still document the repro against 1bc80e1d5.
...
## Report
Write /tmp/slopcop-3408-reports.pPH6eo/issues/<ISSUE>.html. Self-contained HTML (inline CSS, no JS frameworks). Copy the style of /tmp/slopcop-3408-reports.pPH6eo/issues/_template.html (read it). Required sections, in order:
...
5. Minimal reproduction — numbered steps with exact commands, expected vs actual (verbatim output in <pre>), screenshots for visual bugs (with captions describing what to look at). Include the repro test file inline in <pre> AND save it to /tmp/slopcop-3408-reports.pPH6eo/issues/<ISSUE>/repro/ (relative link).
6. Root cause — the mechanism, with code excerpts and permalinks `https://github.com/get-bb/bb/blob/1bc80e1d57991fe60a3896e60e0159427194c7bb/<path>#L<n>-L<m>`. Explain why the visible symptom follows from it. Call out any deeper/underlying issue.
