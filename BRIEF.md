<!-- Values: get-bb/bb · 06aeaa994942ae7527dc49d2268c1f801e8542a0 / 06aeaa994 · 2026-09-08 · /tmp/slopcop-3245-report.VOMa9p/reports · Codex, Claude Code, Pi, Cursor, opencode, Grok Build, and Hermes Agent. -->
...
- Providers available on this machine: Codex, Claude Code, Pi, Cursor, opencode, Grok Build, and Hermes Agent (see `bb provider list`). Real turns cost real usage; keep prompts tiny ("Reply only with ok.").
- Screenshots / browser driving: load the `doobie` skill via the Skill tool (doobie CLI). Save PNGs to /tmp/slopcop-3245-report.VOMa9p/reports/issues/assets/<ISSUE>-<name>.png and reference them as `assets/<ISSUE>-<name>.png`. For visual bugs, capture BEFORE and the moment the bug shows, and a shot of you triggering it (the command/click).
...
- Do NOT touch ~/.bb (the user's real data dir) or the user's real bb instance on :38886. Do NOT push branches or open PRs. Do NOT comment on GitHub. Only write files under /tmp/slopcop-3245-report.VOMa9p/reports/ and inside your worktree.
...
- File hygiene: write ONLY /tmp/slopcop-3245-report.VOMa9p/reports/issues/<ISSUE>.html, /tmp/slopcop-3245-report.VOMa9p/reports/issues/<ISSUE>/** (repro files, logs, diffs) and /tmp/slopcop-3245-report.VOMa9p/reports/issues/assets/<ISSUE>-*.png. Never write loose files in /tmp/slopcop-3245-report.VOMa9p/reports/issues/ root. If files already exist at those paths (a previous attempt), keep their names — never move, rename, or delete existing artifacts; overwrite in place only if you have a better version.
...
## Report
Write /tmp/slopcop-3245-report.VOMa9p/reports/issues/<ISSUE>.html. Self-contained HTML (inline CSS, no JS frameworks). Copy the style of /tmp/slopcop-3245-report.VOMa9p/reports/issues/_template.html (read it). Required sections, in order:
...
5. Minimal reproduction — numbered steps with exact commands, expected vs actual (verbatim output in <pre>), screenshots for visual bugs (with captions describing what to look at). Include the repro test file inline in <pre> AND save it to /tmp/slopcop-3245-report.VOMa9p/reports/issues/<ISSUE>/repro/ (relative link).
